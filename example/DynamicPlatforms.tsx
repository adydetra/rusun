...
export default function DynamicPlatforms() {
  ...
  useFrame((state) => {
    ...
    verticalMovePlatformRef.current?.setNextKinematicRotation(quaternionRotation.setFromAxisAngle(yRotationAxies, time * 0.5));

    // Rotate platform
    rotatePlatformRef.current?.setNextKinematicRotation(quaternionRotation.setFromAxisAngle(yRotationAxies, time * 0.5));

    // Rotate drum
    rotationDrumRef.current?.setNextKinematicRotation(quaternionRotation.setFromAxisAngle(xRotationAxies, time * 0.5));
  });
  ...
  return (
    <>
      ...
      {/* Elevating platform */}
      <RigidBody type="kinematicPosition" position={[-25, 0, 0]} ref={verticalMovePlatformRef} colliders={false}>
        <Text scale={0.5} color="black" maxWidth={10} textAlign="center" position={[0, 2.5, 0]} rotation={[0, Math.PI / 2, 0]}>
      ...
      {/* Rotating drum */}
      <Text scale={0.5} color="black" maxWidth={10} textAlign="center" position={[-15, 2.5, -15]}>
