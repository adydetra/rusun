...
export default function FloatingPlatform() {
  ...
  useFrame(() => {
    ...
    if (floatingPlateRef.current) {
      origin.set(floatingPlateRef.current.translation().x, floatingPlateRef.current.translation().y, floatingPlateRef.current.translation().z);
      rayHit = world.castRay(rayCast, rayLength, false, undefined, undefined, floatingPlateRef.current, floatingPlateRef.current);
    }
    // Ray cast for platform 2
    if (floatingPlateRef2.current) {
      origin2.set(floatingPlateRef2.current.translation().x, floatingPlateRef2.current.translation().y, floatingPlateRef2.current.translation().z);
      rayHit2 = world.castRay(rayCast2, rayLength, false, undefined, undefined, floatingPlateRef2.current, floatingPlateRef2.current);
    }
    // Ray cast for moving platform
    if (floatingMovingPlateRef.current) {
      originMove.set(floatingMovingPlateRef.current.translation().x, floatingMovingPlateRef.current.translation().y, floatingMovingPlateRef.current.translation().z);
      rayHitMove = world.castRay(rayCastMove, rayLength, false, undefined, undefined, floatingMovingPlateRef.current, floatingMovingPlateRef.current);
    ...
    if (rayHit) {
      if (rayHit.collider.parent()) {
        const floatingForce = springK * (floatingDis - rayHit.timeOfImpact) - floatingPlateRef.current.linvel().y * dampingC;
    ...
    if (rayHit2) {
      if (rayHit2.collider.parent()) {
        const floatingForce2 = springK * (floatingDis - rayHit2.timeOfImpact) - floatingPlateRef2.current.linvel().y * dampingC;
    ...
    if (rayHitMove) {
      if (rayHitMove.collider.parent()) {
        const floatingForceMove = springK * (floatingDis - rayHitMove.timeOfImpact) - floatingMovingPlateRef.current.linvel().y * dampingC;
  ...
  return (
    <>
      {/* Platform 1 */}
      <RigidBody position={[0, 5, -10]} mass={1} colliders={false} ref={floatingPlateRef}>
      ...
      {/* Platform 2 */}
      <RigidBody position={[7, 5, -10]} mass={1} colliders={false} ref={floatingPlateRef2}>
        <Text scale={0.5} color="black" maxWidth={10} textAlign="center" position={[0, 2.5, 0]}>
      ...
      <RigidBody position={[0, 5, -17]} mass={1} colliders={false} ref={floatingMovingPlateRef}>
        <Text scale={0.5} color="black" maxWidth={10} textAlign="center" position={[0, 2.5, 0]}>
